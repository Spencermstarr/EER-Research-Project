...
fldpth <- "C:/Users/Spencer/Documents/EER Project/Data/csvs/0-5-1-1 to 0-6-10-500"
...
## Import all 260k of the individual csv files below. In order to 
## accomplish this, I will be using the fread function from the 
## data.table library in R.
## This command reads all of the data in each of the N csv files 
## and stores that data for each of them in their own data.table
...
system.time( datasets <- lapply(datasets, function(dataset_i) { 
...
#save.image("D:/EER/Saved WorkSpaces/Workspaces for dataset folders starting with '0.25'/datasets WorkSpace for '0-5-1-1 to 0-6-10-500'.RData")
save.image("C:/Users/Spencer/Documents/EER Project/Data/WorkSpaces/datasets WorkSpace for '0-5-1-1 to 0-6-10-500'.RData")
stopCluster(CL)
rm(CL)
...
Headers <- c("FS's True Positive Rate", "FS's True Negative Rate", 
             "FS's False Positive Rate")
...
setwd("C:/Users/Spencer/Documents/EER Project/2nd & 3rd Benchmarks/Reproducing the results using the 'MASS' package")
getwd()
df <- data.frame(BE = BE_performance_metrics, FS = FS_performance_metrics)
write.csv(df, 
          file = "stepAIC's Performance on the DSs from 0-5-1-1 to 0-6-10-500.csv",
          row.names = FALSE)
...
write.csv(data.frame(DS_name = DS_names_list, 
                     ...
                     Structural_Variables = sapply(Correctly_Selected_Variables, 
                                                   toString)), 
          file = "Regressors Selected by SR for DSs from 0-5-1-1 to 0-6-10-500.csv",
          row.names = FALSE)
...
save.image("D:/EER folder/WorkSpaces/BE & FS replication WorkSpace for the DSs from 0-5-1-1 to 0-6-10-500 datasets.RData")
