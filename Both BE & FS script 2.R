...
directory_path <- "~/DAEN_698/other datasets/sample obs (alternate)"
...
head(BE_fits)   # returns a list with 18 elements, all of which are NULL
...
head(Variables_selected_by_BE, n = 3)
...
write.csv(data.frame(DS_name = DS_names_list, 
                     Variables_selected = sapply(Variables_selected_by_BE, 
                                                 toString)), 
          file = "BE2_results.csv", row.names = FALSE)
...
write.csv(data.frame(DS_name = DS_names_list, 
                     Variables_selected = sapply(Variables_selected_by_FS, 
                                                 toString)), 
          file = "BE2_results.csv", row.names = FALSE)