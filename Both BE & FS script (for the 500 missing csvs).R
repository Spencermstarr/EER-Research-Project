...
directory_paths <- "~/DAEN_698/other datasets/missing 500 csvs"
...
BE_fits   # returns a list with 18 elements, all of which are NULL
...
write.csv(data.frame(DS_name = DS_names_list, 
                     BE_fits = sapply(BE_fits, toString)), 
          file = "missing_BE_fits.csv", row.names = FALSE)


BE_Coeffs <- lapply(seq_along(BE_fits), function(i) coef(BE_fits[[i]]))
write.csv(data.frame(DS_name = DS_names_list, 
                     BE_Estimates = sapply(BE_Coeffs, toString)), 
          file = "missing_BE_Estimates.csv", row.names = FALSE)


# with the Intercepts included
Models_Selected_by_BE <- lapply(seq_along(BE_fits), 
                              \(i) names(coef(BE_fits[[i]])))

#without the Intercepts included
IVs_Selected_by_BE <- lapply(seq_along(Models_Selected_by_BE), 
                             \(i) Models_Selected_by_BE[[i]][-1])
...
write.csv(data.frame(DS_name = DS_names_list, 
                     Variables_Selected = sapply(IVs_Selected_by_BE, toString)), 
          file = "missing_IVs_Selected_by_BE.csv", row.names = FALSE)
...
### Benchmark Method 3: Run a Forward Selection Stepwise Regression
...
write.csv(data.frame(DS_name = DS_names_list, 
                     FS_fitted_values = sapply(FS_fits, toString)), 
          file = "missing_FS_fits.csv", row.names = FALSE)



FS_Coeffs <- lapply(seq_along(FS_fits), function(i) coef(FS_fits[[i]]))
write.csv(data.frame(DS_name = DS_names_list, 
                     FS_Estimates = sapply(FS_Coeffs, toString)), 
          file = "missing_FS_Estimates.csv", row.names = FALSE)
...
write.csv(data.frame(DS_name = DS_names_list, 
          ...
          file = "missing_IVs_Selected_by_FS.csv", row.names = FALSE)
