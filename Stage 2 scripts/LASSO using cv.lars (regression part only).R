...
IVs.Selected.by.Lars <- lapply(optimal_coefficients, function(i) names(i[i != 0]))
IVs.Not.Selected.by.Lars <- lapply(optimal_coefficients, function(j) names(j[j == 0]))
...
BM1_Accuracy <- lapply(seq_along(datasets), function(i)
  (True_Positives[[i]] + True_Negatives[[i]])/(True_Positives[[i]] + True_Negatives[[i]] + False_Positives[[i]] + BM1_FNs[[i]]))
...
BM1_Recall <- Sensitivity  # You have already calculated recall as True Positive Rate (TPR)
...
PMsC <- data.frame(Num_Under_Correct_or_Over, num_OMVs, num_Extraneous)
