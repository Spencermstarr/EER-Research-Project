...
write.csv(data.frame(DS_name = DS_names_list, 
                     ...
                     Structural_Variables = sapply(Structural_Variables, 
                                                   toString)), 
          file = "Lars's Selections for the DSs from 0.25-7-1-1 to 0.25-7-10-500.csv", 
          row.names = FALSE)
...
Correct <- sum( (TPR == 1) & (FPR == 0) )

# number of models with at least one extraneous variable selected
num_of_Extraneous_IVs <- sum(FPR > 0, na.rm = TRUE)
...
write.csv(Lars_performance, 
          file = "Lars's Performance on the datasets from 0.25-7-1-1 to 0.25-7-10-500.csv", 
          row.names = FALSE)
