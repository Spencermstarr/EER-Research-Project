...
rm(list = ls())
load("C:/Users/Spencer/OneDrive/Documents/Analytics Projects/EER Project/Saved WorkSpaces/Workspaces for dataset folders starting with '0'/datasets WorkSpace for '0-13-1-1 to 0-13-10-500'.RData")
#setwd("C:/Users/Spencer/OneDrive/Documents/Analytics Projects/EER Project/1st Benchmark")
getwd()
...
LASSO_Coeffs <- lapply(LASSO_fits, function(i) predict(i, x = as.matrix(dplyr::select(i, starts_with("X"))), 
                                           s = 0.1, mode = "fraction", 
...
write.csv(data.frame(DS_name = DS_names_list,
          ...
          file = "enet's selections for the DSs from 0-13-1-1 to 0-13-10-500.csv", 
          row.names = FALSE)
...
write.csv(performance_metrics, 
          file = "LASSO's Performance on the datasets from 0-13-1-1 to 0-13-10-500.csv", 
          row.names = FALSE)
