...
BM3_PMs <- read_csv("C:/Users/spenc/Downloads/3rd Benchmark's PMs, grouped by error variance (without subtotals) - csv version.csv")
attach(BM3_PMs)
...
interaction.plot(x.factor = BM3_PMs$Collinearity, 
                 trace.factor = BM3_PMs$`Number of Structural Factors`, 
                 response = BM3_PMs$`Mean TNR`, 
                 xlab = "Collinearity", 
                 ylab = "Specificity",
...
TNR_fit_BM3 <- lm(`Mean TNR` ~ Collinearity + `Number of Structural Factors` +  Collinearity * `Number of Structural Factors` + `Error Variance` + Collinearity * `Error Variance` + `Number of Structural Factors` * `Error Variance`, 
                   data = BM3_PMs)
summary(TNR_fit_BM3)

# create a heatmap
ggplot(BM3_PMs, aes(x = Collinearity, y = `Number of Structural Factors`, fill = `Mean TNR`)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "blue") +
  labs(title = "Heatmap Showing How Collinearity & Num of True Factors Affect the TNR for BM3",
       x = "Collinearity",
...
interaction.plot(x.factor = BM3_PMs$Collinearity, 
                 trace.factor = BM3_PMs$`Number of Structural Factors`, 
                 response = BM3_PMs$`Overspecified Models Selected`, 
                 xlab = "Collinearity", 
...
## run a linear regression instead
Over_fit_BM3 <- lm(`Overspecified Models Selected` ~ Collinearity + `Number of Structural Factors` +  Collinearity * `Number of Structural Factors` + `Error Variance` + Collinearity * `Error Variance` + `Number of Structural Factors` * `Error Variance`, 
                    data = BM3_PMs)
summary(Over_fit_BM3)

# create a heatmap
ggplot(BM3_PMs, aes(x = Collinearity, y = `Number of Structural Factors`, fill = `Overspecified Models Selected`)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "blue") +
  labs(title = "How Collinearity & Num of True Factors Affect the Num of Overspecified Selections for BM3",
       x = "Collinearity",
       y = "Number of Structural Factors",
       fill = "Overspecified Models Selected")

# create an interaction plot
interact_plot(Over_fit_BM3, pred = Collinearity, 
              modx = `Number of Structural Factors`)
...
interaction.plot(x.factor = BM3_PMs$Collinearity, 
                 trace.factor = BM3_PMs$`Number of Structural Factors`, 
                 response = BM3_PMs$`Correctly Specified Models Selected`, 
                 xlab = "Collinearity", 
...
# create a heatmap
ggplot(BM3_PMs, aes(x = Collinearity, y = `Number of Structural Factors`, 
...
anova_collinearity_F1 <- aov(`Mean F1 Score` ~ as.factor(Collinearity), data = BM3_PMs_without_EVs)
summary(anova_collinearity_F1)

# ANOVA for the effect of the Number of Structural Factors on F1-Score
anova_true_factors_F1 <- aov(`Mean F1 Score` ~ as.factor(`Number of Structural Factors`), data = BM3_PMs_without_EVs)
...
anova_both_F1 <- aov(`Mean F1 Score` ~ as.factor(Collinearity) + as.factor(`Number of Structural Factors`), data = BM3_PMs_without_EVs)
