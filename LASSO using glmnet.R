...
library_list <- c(library(plyr),library(dplyr),library(tidyverse),
                  ...
                  library(glmnet),library(vroom),library(data.table),
...
folderpath <- "C:/Users/Spencer/Documents/EER Project/Data/misc. 1000"
...
system.time( datasets2 <- lapply(filepaths_list, read.csv) )
system.time( datasets <- parLapply(CL, filepaths_list, fread))
stopCluster(CL)
class(datasets)   # "list"
str(datasets)

df1 <- read.csv("0-3-1-3.csv", header = FALSE)
colnames(df1) <- c("Y", "X1", "X2", "X3", "X4", "X5", "X6", "X7", 
                  "X8", "X9", "X10", "X11", "X12", "X13", "X14", 
                  "X15", "X16", "X17", "X18", "X19", "X20", "X21", 
                  "X22", "X23", "X24", "X25", "X26", "X27", "X28", 
                  "X29", "X30")
df1_Structural_IVs <- df1[1, -1]
...
LASSO.Coeffs <- lapply(LASSO.fits, function(x){
...
system.time(True_Positives <- lapply(seq_along(datasets), \(i)
                         sum(IVs_Selected_by_LASSO[[i]] %in% 
                               True_Regressors[[i]]))) 


BM1_TPRs = lapply(seq_along(datasets), \(j)
             j <- (True_Positives[[j]]/Total_Positives[[j]]) )

BM1_FPRs = lapply(seq_along(datasets), \(i)
                  i <- length(IVs_Selected_by_LASSO[[i]]) - True_Positives[[i]])
