...
folderpath <- "C:/Users/Spencer/Documents/EER Project/Data/last 50"
...
write.csv(
  data.frame(DS_name = DS_names_list, 
             ...
             Structural_Variables = sapply(Structural_Variables, 
                                           toString)),
  file = "LASSO's Selections via glmnet for the DSs from '0-9-1-1 to 0-9-10-500'.csv", 
  row.names = FALSE)
...
BM1_NNs2 <- lapply(Nonstructural_Variables, function(i) {length(i)})
...
BM1_TPRs2 = lapply(seq_along(datasets), \(j)
...
TPRs2 <- unlist(BM1_TPRs2)
mean_TPR2 <- round(mean(TPRs2), 3)
...
FPRs2 <- unlist(BM1_FPRs2)
mean_FPR <- round(mean(FPRs), 3)
mean_FPR2 <- round(mean(FPRs2), 3)
...
TNRs3 <- unlist(BM1_TNRs3)
mean_TNR2 <- round(mean(TNRs2), 3)
mean_TNR3 <- round(mean(TNRs3), 3)
...
write.csv(performance_metrics, 
          file = "glmnet's Performance on the datasets from '0-9-1-1 to 0-9-10-500'.csv", 
          row.names = FALSE)
